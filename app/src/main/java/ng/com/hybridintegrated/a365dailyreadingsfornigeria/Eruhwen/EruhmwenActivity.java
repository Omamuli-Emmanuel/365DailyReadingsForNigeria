package ng.com.hybridintegrated.a365dailyreadingsfornigeria.Eruhwen;
...
import ng.com.hybridintegrated.a365dailyreadingsfornigeria.edo;
...
public class EruhmwenActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_eruhmwen);

        name="Edo";

        mp=findViewById(R.id.erhumnwen);
        mp.setVisibility(View.VISIBLE);

        mAdview=findViewById(R.id.adderuh);
        AdRequest adRequest=new AdRequest.Builder().build();
        mAdview.loadAd(adRequest);

        mtoolbar=findViewById(R.id.toolbareruh);
        setSupportActionBar(mtoolbar);
        mtoolbar.setNavigationOnClickListener(new View.OnClickListener() {

            @Override
            public void onClick(View view) {
               // startActivity(new Intent(getApplicationContext(),edo.class));
                finish();
            }
        });

        mlist=new ArrayList<>();
        mrecyclervies=(RecyclerView)findViewById(R.id.recyeruh);
        ...
        mref= FirebaseDatabase.getInstance().getReference().child("Eruhmwen");
