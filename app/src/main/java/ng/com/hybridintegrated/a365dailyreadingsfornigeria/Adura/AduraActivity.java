package ng.com.hybridintegrated.a365dailyreadingsfornigeria.Adura;
...
public class AduraActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_adura);


        name="yoruba";

        mp=findViewById(R.id.addura);
        mp.setVisibility(View.VISIBLE);

        mAdview=findViewById(R.id.addadura);
        AdRequest adRequest=new AdRequest.Builder().build();
        mAdview.loadAd(adRequest);

        mtoolbar=findViewById(R.id.toolbaradura);
        setSupportActionBar(mtoolbar);
        mtoolbar.setNavigationOnClickListener(new View.OnClickListener() {

            @Override
            public void onClick(View view) {
               // startActivity(new Intent(getApplicationContext(),yoruba.class));
                finish();
            }
        });

        mlist=new ArrayList<>();
        mrecyclervies=(RecyclerView)findViewById(R.id.recyadura);
        ...
        mref= FirebaseDatabase.getInstance().getReference().child("Adura");
