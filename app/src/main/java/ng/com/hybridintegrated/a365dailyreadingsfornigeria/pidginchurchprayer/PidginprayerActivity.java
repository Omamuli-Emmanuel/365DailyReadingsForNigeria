package ng.com.hybridintegrated.a365dailyreadingsfornigeria.pidginchurchprayer;
...
import ng.com.hybridintegrated.a365dailyreadingsfornigeria.pidginLangg;
...
public class PidginprayerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pidginprayer);

        name="pidgin";

        mp=findViewById(R.id.ppd);
        mp.setVisibility(View.VISIBLE);

        mAdview=findViewById(R.id.addpidgins);
        AdRequest adRequest=new AdRequest.Builder().build();
        mAdview.loadAd(adRequest);

        mtoolbar=findViewById(R.id.toolbarpidgins);
        setSupportActionBar(mtoolbar);
        mtoolbar.setNavigationOnClickListener(new View.OnClickListener() {

            @Override
            public void onClick(View view) {
                //startActivity(new Intent(getApplicationContext(),pidginLangg.class));
                finish();
            }
        });

        mlist=new ArrayList<>();
        mrecyclervies=(RecyclerView)findViewById(R.id.recypidgins);
        ...
        mref= FirebaseDatabase.getInstance().getReference().child("pidginchurchprayer");
